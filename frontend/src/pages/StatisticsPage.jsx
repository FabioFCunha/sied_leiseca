...
const text = {
  ...
  kitsWithSevenComics: "Kits com 7 Revistinhas", kitsHint: "Total de kits distribuídos", internal: "Solicitações internas", public: "Solicitações públicas", internalHint: "Agendas criadas pela equipe",
...
const KPI_DEFS = [
  ...
  ["MATERIAL - Geral", text.materials, BookOpen, COLORS[5], "Total de materiais para distribuição"],
  ["MATERIAL - Kit com 7 Revistinhas", text.kitsWithSevenComics, BookOpen, COLORS[7], text.kitsHint],
...
export default function StatisticsPage() {
  ...
  return <section className="stats-dashboard" ref={dashboardRef}><header className="stats-hero"><div><span>{"Inteligência institucional"}</span><h1>Dashboard Executivo SIED</h1><p>{"Indicadores oficiais a partir de 09/07/2026, quando o sistema passou a operar."}</p></div><div className="stats-export"><button onClick={exportPdf}><Printer size={16}/>PDF</button><button onClick={exportExcel}><FileSpreadsheet size={16}/>Excel</button><button onClick={exportCsv}><Download size={16}/>CSV</button><button onClick={exportImage}><FileImage size={16}/>Imagem</button></div></header>
    ...
    <p className="stats-kpi-source">Fonte: relatórios técnicos aprovados no SIED. Materiais distribuídos contabilizados a partir de 09/07/2026.</p>
    ...
    <div className="stats-two-columns stats-street-admin-layout"><Section icon={BarChart3} title={"Ações de rua"} subtitle="Somente categorias com movimento." className="stats-street-panel"><ExecutiveBar rows={categoryData.map(row => ({ label: row.label, actions: row.value, audience: row.audience, color: row.color, onClick: () => setDrilldown(row) }))}/></Section><div className="stats-right-stack"><Section icon={Building2} title={"Demandas Administrativas"} subtitle={"Solicitações internas por subtipo."}><ExecutiveBar rows={administrativeDemandRows}/></Section><Section icon={Building2} title={"Convênios educacionais"} subtitle={"Ações e pessoas por indicador institucional."}><ExecutiveBar rows={educationalAgreementRows}/></Section><Section icon={Users} title={"Faixa etária do público"} subtitle={"Perfil informado na solicitação."}><ExecutiveBar rows={ageRows}/></Section></div></div>
