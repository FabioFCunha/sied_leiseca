...
const cardConfig = [
  { key: "approved", label: "Aguardando OS", tooltip: "Solicita√ß√µes aprovadas que ainda aguardam a gera√ß√£o da Ordem de Servi√ßo", icon: CheckCircle2, tone: "green", status: "APPROVED", color: "#00b894", gradient: "linear-gradient(135deg, #00b894, #009472)" },
  { key: "pending", label: "Aguardando an√°lise", tooltip: "Solicita√ß√µes ainda n√£o avaliadas pelo Gestor ou Administrador", icon: Clock3, tone: "amber", status: "PENDING", color: "#fdcb6e", gradient: "linear-gradient(135deg, #fdcb6e, #e1b12c)" },
  { key: "cancelled", label: "Recusadas / Canceladas", tooltip: "Solicita√ß√µes recusadas ou canceladas no per√≠odo selecionado", icon: XCircle, tone: "red", status: "CANCELLED", color: "#d63031", gradient: "linear-gradient(135deg, #d63031, #b33939)" },
  { key: "completed", label: "Relat√≥rios aprovados", tooltip: "A√ß√µes com relat√≥rio t√©cnico conferido e aprovado", icon: CheckCheck, tone: "emerald", status: "COMPLETED", color: "#0984e3", gradient: "linear-gradient(135deg, #0984e3, #0762a8)" },
  { key: "upcoming", label: "Pr√≥ximas agendas", icon: CalendarClock, tone: "violet", color: "#6c5ce7", gradient: "linear-gradient(135deg, #6c5ce7, #5345b5)" },
...
const chartFilters = ["Hoje", "Semana", "M√™s", "Ano"];
...
const exceptionalOccurrenceTypeLabels = {
  ...
  MEDICAL_ASSISTANCE: "Atendimento m√©dico",
  EQUIPMENT_OR_MATERIAL: "Problema com equipamento ou material",
  ACTIVITY_INTERRUPTION: "Interrup√ß√£o da atividade",
  SECURITY_INCIDENT: "Ocorr√™ncia de seguran√ßa",
  SEVERE_WEATHER: "Condi√ß√£o clim√°tica grave",
  OTHER: "Outro",
};

const exceptionalOccurrenceImpactLabels = {
  NO_IMPACT: "Sem preju√≠zo",
...
const chiefComparisonConfig = [
  {
    ...
    tooltip: "Total de pessoas abordadas em todas as a√ß√µes enviadas",
    icon: Users,
    reported: "approaches",
    color: "#0048d7",
  ...
  {
    key: "actions",
    label: "A√ß√µes Realizadas",
    tooltip: "A√ß√µes executadas, independentemente da aprova√ß√£o do relat√≥rio",
    icon: CalendarCheck,
  ...
  },
  {
    ...
    tooltip: "A√ß√µes realizadas cujos relat√≥rios ainda n√£o foram aprovados",
    icon: Clock3,
  ...
  {
    key: "avg-action",
    label: "M√©dia por A√ß√£o",
    tooltip: "M√©dia de abordagens por a√ß√£o realizada",
    icon: Activity,
  ...
  {
    key: "avg-team",
    label: "M√©dia por Equipe",
    tooltip: "M√©dia de abordagens dividida pelo n√∫mero de equipes",
    icon: Shield,
    ...
    <div className="chart-card chief-fillings-card" style={{ border: "1px solid var(--line)", borderRadius: "16px", padding: "24px", marginBottom: "32px", background: "var(--surface)" }}>
      <div className="section-heading" style={{ marginBottom: "20px" }}>
        <div>
          ...
          <p style={{ fontSize: "13px", color: "var(--text-soft)", marginTop: "4px" }}>Vis√£o direta e consolidada do resultado das equipes em campo.</p>
        </div>
      </div>
...
function LineChart({ data = [] }) {
  ...
  return (
    <div className="chart-card main-chart" style={{ border: "1px solid var(--line)", borderRadius: "16px", padding: "20px" }}>
      <div className="section-heading" style={{ marginBottom: "20px" }}>
        <div>
          <h2 style={{ fontSize: "16px", fontWeight: "800" }}>Evolu√ß√£o das agendas</h2>
          <p style={{ fontSize: "12px", color: "var(--text-soft)" }}>Comparativo di√°rio do per√≠odo selecionado.</p>
        </div>
      </div>
  ...
  return (
    <div className="chart-card donut-card" style={{ border: "1px solid var(--line)", borderRadius: "16px", padding: "20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div className="section-heading" style={{ alignSelf: "stretch", marginBottom: "16px" }}>
        <h2 style={{ fontSize: "15px", fontWeight: "800" }}>Agendas por munic√≠pio</h2>
      </div>
...
function Heatmap({ data = [] }) {
  ...
  return (
    <div className="chart-card heatmap-card" style={{ border: "1px solid var(--line)", borderRadius: "16px", padding: "20px" }}>
      <div className="section-heading" style={{ marginBottom: "16px" }}>
        <h2 style={{ fontSize: "15px", fontWeight: "800" }}>Hor√°rios mais utilizados</h2>
      </div>
    ...
    <aside className="dashboard-side" style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div className="chart-card materials-dashboard-card" style={{ border: "1px solid var(--line)", borderRadius: "16px", padding: "20px" }}>
        <div className="section-heading" style={{ marginBottom: "14px" }}>
          <div>
            <h2 style={{ fontSize: "15px", fontWeight: "800", textTransform: "uppercase" }}>Materiais distribu√≠dos</h2>
            <p style={{ color: "var(--text-soft)", fontSize: "12px", fontWeight: "700", margin: "3px 0 0", textTransform: "uppercase" }}>Total consolidado dos relat√≥rios enviados.</p>
          </div>
        ...
        <div className="advanced-list" style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {distributedMaterials.items?.length ? distributedMaterials.items.map((item) => (
            ...
            <p style={{ color: "var(--text-soft)", fontSize: "13px", fontWeight: "800", margin: 0, textTransform: "uppercase" }}>Nenhum material distribu√≠do registrado.</p>
          )}
        </div>
      </div>
      <div className="chart-card advanced-indicators-card" style={{ border: "1px solid var(--line)", borderRadius: "16px", padding: "20px" }}>
        <div className="section-heading" style={{ marginBottom: "14px" }}><h2 style={{ fontSize: "15px", fontWeight: "800" }}>Indicadores avan√ßados</h2></div>
        <div className="advanced-list" style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <span style={{ display: "flex", justifyContent: "space-between", fontSize: "13px" }}>Taxa de aprova√ß√£o <strong style={{ color: "var(--success)" }}>{advanced?.approval_rate ?? 0}%</strong></span>
          ...
          <span style={{ display: "flex", justifyContent: "space-between", fontSize: "13px" }}>Tempo m√©dio de aprova√ß√£o <strong>{advanced?.approval_avg_hours ?? 0}h</strong></span>
          <span style={{ display: "flex", justifyContent: "space-between", fontSize: "13px" }}>M√©dia por usu√°rio <strong>{advanced?.avg_per_user ?? 0}</strong></span>
...
const reportStatusStyle = {
  none: { label: "Sem relat√≥rio", color: "#6b7280", bg: "#f3f4f6" },
  draft: { label: "Rascunho", color: "#b45309", bg: "#fef3c7" },
  pending_review: { label: "Aguardando confer√™ncia", color: "#1d4ed8", bg: "#dbeafe" },
...
function ChiefNarrative({ text }) {
  const narrative = String(text || "").trim() || "Relato ainda n√£o informado.";
...
function OperationalSummaryPanel({ summary = {} }) {
  const items = [
    { label: "A√ß√µes programadas", value: summary.scheduled_today },
    { label: "Realizadas", value: summary.completed },
    { label: "Em andamento", value: summary.in_progress },
    { label: "Pr√≥ximas", value: summary.pending_start },
  ...
  return (
    <SectionCard icon={Flag} title={"Resumo operacional do dia"} subtitle={"Leitura direta do volume programado, andamento e mobiliza√ß√£o do per√≠odo."}>
...
function OperationDashboardTable({ operations = [] }) {
  return (
    <SectionCard icon={MapPin} title={"OperaÁ„o do dia"} subtitle={"Hor·rio, local, efetivo, frequÍncia, relatÛrio e relato do chefe em leitura operacional."}>
      {operations.length ? (
            ...
            <article key={item.id} style={{ border: "1px solid var(--line)", borderRadius: 16, padding: "18px", background: "var(--surface-2)", display: "grid", gap: 14 }}>
                      ...
                      <span style={{ color: "var(--text)", fontSize: 13.5, lineHeight: 1.45 }}>
                        {item.service_order_mode === "DESIGNATED"
                          ? item.designated_users_names?.length
                            ...
                            : "Participantes ainda n„o informados"
                          : `Equipe ${item.team} ∑ Chefe: ${item.chief}`}
                      </span>
                    </div>
                    <div style={{ border: "1px solid var(--line)", borderRadius: 12, background: "var(--surface)", padding: "12px 14px", display: "grid", gap: 5 }}>
                      <strong style={{ color: "var(--text)", fontSize: 12, textTransform: "uppercase" }}>P˙blico previsto</strong>
                      <span style={{ color: "var(--text)", fontSize: 13.5 }}>{formatAudienceValue(item.estimated_public)}</span>
                    </div>
                    <div style={{ border: "1px solid var(--line)", borderRadius: 12, background: "var(--surface)", padding: "12px 14px", display: "grid", gap: 5 }}>
                      <strong style={{ color: "var(--text)", fontSize: 12, textTransform: "uppercase" }}>P˙blico alcanÁado</strong>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <small style={{ color: "var(--text-soft)" }}>
                  <strong style={{ color: "var(--text)" }}>FrequÍncia:</strong> {item.attendance_status_label} {" ∑ "}
                  <strong style={{ color: "var(--text)" }}>RelatÛrio:</strong> {item.report_status_label}
                </small>
                {item.href ? (
        ...
        <p style={{ margin: 0, color: "var(--text-soft)", fontWeight: 700 }}>{"Nenhuma aÁ„o programada para hoje."}</p>
      )}
    </SectionCard>
  );
}
...
function buildOperationalAttentionItems(alerts = []) {
  ...
  return safeAlerts.map((item, index) => ({
    title: item?.title || `PendÍncia ${index + 1}`,
...
function AttentionPanel({ alerts = [] }) {
  ...
  return (
    <SectionCard icon={Activity} title={"PendÍncias e atenÁ„o"} subtitle={"PendÍncias objetivas do perÌodo com acesso r·pido ‡s operaÁıes afetadas."}>
      {insights.length ? (
        <div style={{ display: "grid", gap: 12 }}>
        ...
        <p style={{ margin: 0, color: "var(--text-soft)", fontWeight: 700 }}>N„o h· pendÍncias para os filtros selecionados.</p>
      )}
    </SectionCard>
  );
}
...
function NextOperationsPanel({ operations = [] }) {
  return (
    <SectionCard icon={CalendarClock} title={"PrÛximas operaÁıes"} subtitle={"AÁıes j· classificadas como prÛximas dentro do mesmo conjunto filtrado."}>
      {operations.length ? (
        ...
        <p style={{ margin: 0, color: "var(--text-soft)", fontWeight: 700 }}>Nenhuma prÛxima operaÁ„o para os filtros selecionados.</p>
      )}
    </SectionCard>
  );
}
...
function OperationalClosingPanel({ closing = {} }) {
  ...
  return (
    <SectionCard icon={FileText} title={"Fechamento operacional"} subtitle={"ConsolidaÁ„o operacional do perÌodo filtrado, sem transformar o dashboard em estatÌstica institucional."}>
      <div style={{ display: "grid", gap: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
          {[
            ["AÁıes programadas", closing.scheduled_today],
            ["Realizadas", closing.completed],
            ["Em andamento", closing.in_progress],
            ["PrÛximas", closing.pending_start],
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
          <div style={{ border: "1px solid var(--line)", borderRadius: 12, padding: "14px", background: "var(--surface-2)", display: "grid", gap: 6 }}>
            <strong style={{ fontSize: 12, color: "var(--text-soft)", textTransform: "uppercase" }}>FrequÍncias</strong>
            <span style={{ color: "var(--text)", fontSize: 14 }}>ConcluÌdas: {Number(attendance.completed || 0).toLocaleString("pt-BR")}/{Number(attendance.total || 0).toLocaleString("pt-BR")}</span>
          ...
          <div style={{ border: "1px solid var(--line)", borderRadius: 12, padding: "14px", background: "var(--surface-2)", display: "grid", gap: 6 }}>
            <strong style={{ fontSize: 12, color: "var(--text-soft)", textTransform: "uppercase" }}>RelatÛrios</strong>
            <span style={{ color: "var(--text)", fontSize: 14 }}>Sem relatÛrio: {Number(reports.none || 0).toLocaleString("pt-BR")} ∑ Rascunho: {Number(reports.draft || 0).toLocaleString("pt-BR")}</span>
            <span style={{ color: "var(--text-soft)", fontSize: 13 }}>ConferÍncia: {Number(reports.pending_review || 0).toLocaleString("pt-BR")} ∑ Devolvidos: {Number(reports.returned || 0).toLocaleString("pt-BR")} ∑ Aprovados: {Number(reports.approved || 0).toLocaleString("pt-BR")}</span>
          </div>
          <div style={{ border: "1px solid var(--line)", borderRadius: 12, padding: "14px", background: "var(--surface-2)", display: "grid", gap: 6 }}>
            <strong style={{ fontSize: 12, color: "var(--text-soft)", textTransform: "uppercase" }}>P˙blico</strong>
...
function WorkforcePanel({ cards = {} }) {
  return (
    <SectionCard icon={Users} title="Efetivo do dia" subtitle="Recursos operacionais vinculados √†s a√ß√µes de hoje.">
...
export default function DashboardPage() {
  ...
  const [chartRange, setChartRange] = useState("M√™s");
  ...
  const handleExport = async (format, e) => {
    ...
    try {
      ...
      console.error(err);
      alert(err.message || "Erro ao exportar o relat√≥rio.");
    }
  };
  ...
  return (
    <section className="page dashboard-page">
      <div className="dashboard-hero" style={{ background: "linear-gradient(135deg, #001338 0%, #002d72 100%)", padding: "28px", borderRadius: "16px", color: "#ffffff", marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 8px 32px 0 rgba(0, 19, 56, 0.15)" }}>
        <div>
          <span style={{ fontSize: "11px", fontWeight: "800", textTransform: "uppercase", letterSpacing: "1.5px", color: "#f6bd16", opacity: 0.95, display: "block", marginBottom: "2px" }}>Vis√£o operacional</span>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
            ...
            {isModerator && dashboard?.pending_moderation_count > 0 && (
              ...
              >
                {dashboard.pending_moderation_count} avalia√ß√µes pendentes
              </span>
            )}
          </div>
          <p style={{ margin: 0, color: "#d2e1ff", opacity: 0.9, fontSize: "15px", marginTop: "8px" }}>SISTEMA INTEGRADO DA EDUCA√á√ÉO - OPERA√á√ÉO LEI SECA</p>
        </div>
      ...
      <div className="global-filters" style={{ border: "1px solid var(--line)", background: "var(--surface)", borderRadius: "14px", padding: "16px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "16px", marginBottom: "24px", alignItems: "end" }}>
        ...
        <label className="filter-field" style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          <span style={{ fontSize: "11px", fontWeight: "700", color: "var(--text-soft)", textTransform: "uppercase" }}>Munic√≠pio</span>
      ...
      {loading ? (
        ...
        <div className="alert">N√£o foi poss√≠vel carregar o Dashboard: {error}</div>
      ) : (
        <>
