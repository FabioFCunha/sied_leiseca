...
export default function OperationalMap({ operations = [], onOpenAgenda }) {
  ...
  const points = useMemo(() => operations.map((item) => {
    if (item.latitude == null || item.longitude == null || item.latitude === "" || item.longitude === "") return null;
    const latitude = Number(item.latitude);
    const longitude = Number(item.longitude);
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude) || latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) return null;
    return { ...item, position: [latitude, longitude] };
  }).filter(Boolean), [operations]);
  const pointIds = useMemo(() => new Set(points.map((item) => item.id)), [points]);
  const unlocatedCount = operations.length - points.length;
  const focus = (item) => {
  ...
  return <section className="operational-map-card" aria-labelledby="operational-map-title">
    <header className="operational-map-heading"><div><span>Monitoramento em campo</span><h2 id="operational-map-title">Mapa Operacional de Hoje</h2><p>{operations.length} ações distribuídas em {municipalities} municípios • {teams} equipes em campo</p></div>{unlocatedCount > 0 && <small>{unlocatedCount} {unlocatedCount === 1 ? "endereço não localizado" : "endereços não localizados"}</small>}</header>
    <div className="operational-map-layout">
      <aside className="operational-map-list" aria-label="Operação em campo"><h3>Operação em Campo</h3>
        {operations.length ? operations.map((item) => { const [label, color] = statusOf(item); const found = pointIds.has(item.id); return <button type="button" key={item.id} className={selected === item.id ? "is-active" : ""} onClick={() => focus(item)} disabled={!found} title={found ? `Localizar: ${label}` : "Endereço não localizado"}><i style={{ background: color }} /><strong>{item.time || "--:--"}</strong><span>{item.location || item.title}</span><small>{item.team}</small></button>; }) : <p>Nenhuma ação no período selecionado.</p>}
      </aside>
      <div className="operational-map-canvas">
        ...
        {!points.length && operations.length > 0 && <div className="operational-map-no-points">Nenhum ponto localizado • {unlocatedCount} {unlocatedCount === 1 ? "endereço não localizado" : "endereços não localizados"}</div>}
      </div>
    </div>
  </section>;
}
