...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--file",
            required=True,
            help="Caminho para o arquivo XLSX historico.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
            help="Executa somente a leitura e validacao do arquivo.",
        )
        parser.add_argument(
            "--apply",
            action="store_true",
            help="Aplica a importacao real no banco de dados.",
        )
        parser.add_argument(
            "--source-type",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if not is_dry_run and not is_apply:
            raise CommandError(
                "Voce deve especificar --dry-run ou --apply."
            )

        if is_apply and is_dry_run:
            raise CommandError(
                "Nao pode especificar --dry-run e --apply simultaneamente."
            )
        ...
        if is_apply:
            ...
            from apps.inspection.historical_import import (
                InspectionHistoricalImportService,
            )
